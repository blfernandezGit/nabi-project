...
const Index = ({ code, ticket_no, handleclose, getUpdatedTicket, ticketData }) => {
    ...
    useEffect(() => {
        setProjectTickets(
            ...
            .filter(ticket => !ticketData?.attributes?.related_tickets?.map(relatedTicket => { return relatedTicket?.id }).includes(ticket?.id))
            .filter(ticket => !ticketData?.attributes?.inverse_related_tickets?.map(inverseRelatedTicket => { return inverseRelatedTicket?.id }).includes(ticket?.id))
            .map( ticket => {
                return ({
